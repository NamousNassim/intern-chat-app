...
app.get("/api/users", (req, res) => {
  db.all("SELECT id, username, profilePic, isAdmin, createdAt FROM users ORDER BY createdAt DESC", (err, users) => {
    if (err) {
...
app.post("/api/users", profileUpload.single("profilePic"), (req, res) => {
  ...
  db.run(
    ...
    [username, hashedPassword, profilePic, isAdmin === "true" || isAdmin === true],
    function(err) {
      if (err) {
      ...
      console.log(`✅ Utilisateur créé: ${username} (ID: ${this.lastID})`);
...
app.delete("/api/users/:id", (req, res) => {
  ...
  if (!userId || userId === 1) {
    return res.status(400).json({ error: "Impossible de supprimer cet utilisateur" });
  }
  
  // Récupérer l'utilisateur pour supprimer sa photo de profil si c'est une image générée
  db.get("SELECT profilePic, username FROM users WHERE id = ?", [userId], (err, user) => {
    if (err) {
    ...
    db.run("DELETE FROM users WHERE id = ?", [userId], function(err) {
      ...
      if (user.profilePic && user.profilePic.includes('profile_')) {
        const imagePath = path.join(__dirname, 'public', user.profilePic);
        if (fs.existsSync(imagePath)) {
          try {
            fs.unlinkSync(imagePath);
            console.log(`🗑️ Image supprimée: ${user.profilePic}`);
          } catch (fsErr) {
            console.log("⚠️ Erreur suppression image:", fsErr);
          }
        }
      }
      
      console.log(`✅ Utilisateur supprimé: ${user.username} (ID: ${userId})`);
...
app.get("/api/messages", (req, res) => {
  db.all(
    ...
    (err, messages) => {
      if (err) {
        console.log("❌ Erreur récupération messages:", err);
        return res.status(500).json({ error: "Erreur serveur" });
      }
      res.json(messages || []);
    }
